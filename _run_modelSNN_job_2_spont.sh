...
for modules in '2' '1'; do
    ...
    # -----------------------------
    python3.9 -u _run_modelSNN_2_sim_spontaneous.py $path_results $modules $dist_rule $nNeurons $local_II $tau_AMPA $tau_GABA $tau_R $beta_E $beta_I $gain_AMPA $gain_GABA $time_sec $g_E $g_I $I_intensity_exc $I_intensity_inh $rate_exc $rate_inh $out_name > "_logs/_2_logs_jobs_spontaneous/output_"${out_name}".log" 2> "_logs/_2_logs_jobs_spontaneous/error_"${out_name}".log"
