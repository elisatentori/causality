...
array_config="./_run_modelSNN_config_sim_2.txt"

mkdir -p _logs/
mkdir -p _logs/_3_logs_jobs_evoked
...
for modules in '2' '1'; do
    ...
    # -----------------------------
    python3.9 -u _run_modelSNN_3_sim_evoked.py $path_results $modules $dist_rule $nNeurons $local_II $tau_AMPA $tau_GABA $tau_R $beta_E $beta_I $gain_AMPA $gain_GABA $g_E $g_I $I_intensity_exc $I_intensity_inh $rate_exc $rate_inh $Ntrials $dT_trial_sec $IPI_ms $n_pulses $alpha_th $Tmax $out_name > "_logs/_3_logs_jobs_evoked/output_"${out_name}".log" 2> "_logs/_3_logs_jobs_evoked/error_"${out_name}".log"
