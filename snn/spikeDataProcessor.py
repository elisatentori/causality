...
class SpikeDataProcessor:
    ...
    def _load_spike_data(self):
        ...
        def load_spikeTimes(main_path, T_cutoff=10000, stim_ST=None, stim_CH=None):
            ...
            spont_mask = cols < self.T_cutoff * self.fs  # solo spontaneo
            spont_times = cols[spont_mask] / self.fs
            spont_chans = (rows[spont_mask] - 1).astype(int)
    
            if stim_ST is not None and stim_CH is not None:
                all_times = np.concatenate([spont_times, stim_ST / self.fs]).astype(float)
                all_chans = np.concatenate([spont_chans, stim_CH]).astype(int)
                return all_times, all_chans
            else:
                return spont_times, spont_chans

        path         = self.main_path + self.sim_folder + 'Data/' + self.spike_file
        ...
        spont_mask   = self.spikes[0] < self.T_cutoff
        self.rate    = self._compute_firing_rate((self.spikes[0][spont_mask], self.spikes[1][spont_mask]))
